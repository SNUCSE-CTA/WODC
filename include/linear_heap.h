#ifndef LINEAR_HEAP_H_
#define LINEAR_HEAP_H_
...
class ListLinearHeap {
 ...
 public:
  ...
  void get_min_key_ids(ui *vs, ui &vs_size) {
    while (min_key <= max_key && head_s[min_key] == n) ++min_key;

    for (ui id = head_s[min_key]; id != n; id = next_s[id]) {
      vs[vs_size++] = id;
    }
  }

  bool get_min(ui &id,
               ui &key) {  // return true if success, return false otherwise
    while (min_key <= max_key && head_s[min_key] == n) ++min_key;
  ...
  bool pop_min(ui &id, ui &key, ui num) {
    while (min_key <= max_key && head_s[min_key] == n) ++min_key;
    if (min_key > max_key) return false;
